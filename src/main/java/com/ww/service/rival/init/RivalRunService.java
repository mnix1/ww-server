...
import com.ww.model.container.rival.init.*;
...
public class RivalRunService {
    ...
    RivalManager createManager(RivalInit initContainer) {
        ...
        if (type == RivalType.WAR) {
            return new WarManager((RivalTwoPlayerInit) initContainer, rivalWarService, profileConnectionService);
        }
        if (type == RivalType.CAMPAIGN_WAR) {
            return new CampaignWarManager((RivalCampaignWarInit) initContainer, rivalCampaignWarService, profileConnectionService);
        }
        if (type == RivalType.BATTLE) {
            return new BattleManager((RivalTwoPlayerInit) initContainer, rivalBattleService, profileConnectionService);
        }
        if (type == RivalType.CHALLENGE) {
            return new ChallengeManager((RivalChallengeInit) initContainer, rivalChallengeService, profileConnectionService);
        }
