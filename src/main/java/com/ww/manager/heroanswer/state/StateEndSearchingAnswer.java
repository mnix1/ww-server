...
public class StateEndSearchingAnswer extends State {
    protected static final Logger logger = LoggerFactory.getLogger(StateEndSearchingAnswer.class);
    private double getMinEndSearchingAnswerInterval() {
        return manager.getQuestion().getAnswers().size() * 0.5;
    }

    public StateEndSearchingAnswer(HeroAnswerManager manager) {
        super(manager);
    }

    protected Flowable<Long> processFlowable() {
        manager.addAndSendAction(HeroAnswerAction.SEARCHING_FOR_ANSWER);
